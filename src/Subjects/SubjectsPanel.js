...
export class SubjectsPanel extends Component {
  state = {
    loading: false,
    subjects: [],
    searchText: "",
  ...
  deleteSubject = async (subjectId, subjectName) => {
    if (
      window.confirm(
        `Do you want to delete a subject with name ${subjectName} ?`
      )
    ) {
      try {
        await axios.delete(
          `https://english-center.onrender.com/api/v1/subjects/delete/${subjectId}`
        );
        this.getSubjects();
      } catch (error) {
        console.log(error);
  ...
  search = async () => {
    ...
    try {
      const res = await axios.get(
        `https://english-center.onrender.com/api/v1/subjects/search/${this.state.searchText}`
      );
      this.setState({ loading: false });
