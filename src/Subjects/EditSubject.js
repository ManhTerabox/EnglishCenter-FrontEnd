...
export default class EditSubject extends Component {
  ...
  getSubject = async (e) => {
    ...
    try {
      const res = await axios.get(
        `https://english-center.onrender.com/api/v1/subject/${subjectId}`
      );
  ...
  editSubjectHandler = async (e) => {
    ...
    try {
      ...
      console.log(body);
      const addingResult = await axios.patch(
        "https://english-center.onrender.com/api/v1/subjects/edit",
        body
      );
  ...
  render() {
    return (
      <Fragment>
        <Intro
          logo="School"
          thisCategory={`Edit ${this.state.name} Subject `}
        />
