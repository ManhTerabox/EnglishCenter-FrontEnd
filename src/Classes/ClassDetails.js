...
export default class ClassDetails extends Component {
  ...
  getClass = async () => {
    try {
      ...
      const res = await axios.get(
        `https://english-center.onrender.com/api/v1/class/${classId}`
      );
  ...
  removeStudentFromClass = async (studentId, studentName) => {
    ...
    try {
      if (
        window.confirm(
          `Are you sure you want to remove a student with name ${studentName} from this class ?`
        )
      ) {
        const body = { classId: classId, studentId: studentId };
        await axios.patch(
          `https://english-center.onrender.com/api/v1/class/removeStudent`,
          body
        );
        this.getClass();
      }
      ...
      <div>
        {this.state.classObj && (
          <div>
            ...
            <div>
              <h2>Class Code : {this.state.classObj.code}</h2>
            ...
            {this.state.classObj.realStudents.length > 0 ? (
              ...
              >{`${this.state.classObj.name} Class has no students `}</h1>
            )}
          </div>
        )}
