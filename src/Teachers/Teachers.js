...
class Teachers extends Component {
  ...
  getTeachers = async () => {
    try {
      ...
      const res = await axios.get(
        "https://english-center.onrender.com/api/v1/teachers"
      );
  ...
  deleteTeacher = async (teacherId, teacherName) => {
    if (
      window.confirm(
        `Are you sure you want to delete a teacher with name ${teacherName} ?`
      )
    ) {
      try {
        await axios.delete(
          `https://english-center.onrender.com/api/v1/teachers/delete/${teacherId}`
        );
        this.getTeachers();
      } catch (error) {
        console.log(error);
  ...
  search = async () => {
    ...
    try {
      const res = await axios.get(
        `https://english-center.onrender.com/api/v1/teachers/search/${this.state.searchText}`
      );
      this.setState({ loading: false });
