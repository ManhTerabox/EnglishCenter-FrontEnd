...
export class EditTeacher extends Component {
  ...
  getTeacher = async () => {
    ...
    try {
      const res = await axios.get(
        `https://english-center.onrender.com/api/v1/teachers/${teacherId}`
      );

      console.log("addTeachers -> getTeacher -> res", res);

      const { firstName, lastName, age, gender, email, subjectId, salary } =
        res.data.teacher;
      this.setState({
        firstName: firstName,
        lastName: lastName,
  ...
  getSubjectsForSelecting = async () => {
    try {
      const res = await axios.get(
        "https://english-center.onrender.com/api/v1/subjects"
      );
  ...
  submitHandler = async (e) => {
    ...
    try {
      const { firstName, lastName, age, gender, email, salary, subjectId } =
        this.state;
      const body = {
        _id: teacherId,
        firstName: firstName,
      ...
      const res = await axios.patch(
        "https://english-center.onrender.com/api/v1/teacher/edit",
        body
      );
      console.log(res);
  ...
  render() {
    return (
      <Fragment>
        ...
        {this.state.doneObj && (
          <MainAlert
            message={this.state.doneObj.message}
            type={this.state.doneObj.type}
          />
        )}
      </Fragment>
    );
